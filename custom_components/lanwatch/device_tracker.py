...
class LanwatchTracker(CoordinatorEntity[LanwatchCoordinator], ScannerEntity):
    ...
    def _generate_device_name(self, device_info: LanwatchDeviceInfo | None) -> str:
        ...
        if device_info.name:
            ...
            if (
                hostname
                and not hostname.replace(".", "").replace("-", "").isdigit()
                and hostname.lower() not in ["localhost", "unknown", "_gateway", "android"]
            ):
                # Clean up hostname
                clean_name = hostname.replace("-", " ").replace("_", " ")
                # Handle special cases
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_name != self._attr_name:
            ...
            if (
                "Unknown" in self._attr_name
                or "Device" in self._attr_name
                and self._mac[-8:].replace(":", "") in self._attr_name
            ):
